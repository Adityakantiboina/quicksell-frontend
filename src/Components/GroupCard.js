...
const GroupCard = () => {
  ...
  const [userColorMapping, setUserColorMapping] = useState({});
      ...
      {ticketGroup.map((ticket) => (
        <div key={ticket.id} className="ticket">
          ...
          <p style={{ color: "black", margin: "0"}}>
            {groupBy !== "status" && <img src={priorityMapping[ticket.priority]?.icon} alt={priorityMapping[ticket.priority]?.label} style={{ marginRight: '5px' }} />}
            {ticket.title}
          </p>
  ...
  return (
    <div>
      {/* Navbar with Display Button */}
      <div className="navbar">
        <button
          className="display-button"
          onClick={() => setIsOpen(!isOpen)}
        >
          <img src={DisplaySVG} alt="Display" style={{ marginRight: '5px' }} />
          Display
        </button>
      </div>
        ...
        {groupBy === "status" && statusCategories.map(({ name, icon }) => (
          <div key={name} style={{ flex: "1", padding: "10px", position: "relative" }}>
            <p style={{ display: "flex", alignItems: "center" }}>
              ...
              <img src={AddSVG} alt="Add" className="add-button" onClick={() => console.log(`Add ticket to ${name}`)} />
              <img src={Threedot} alt="Options" className="add-button" onClick={() => console.log(`Options for ${name}`)} />
            </p>
        ...
        {groupBy === "userId" && Object.keys(tickets).map((userId) => (
          <div key={userId} style={{ flex: "1", padding: "10px", position: "relative" }}>
            <p style={{ display: "flex", alignItems: "center" }}>
              ...
              >
                {data.users && userId ? data.users.find(user => user.id === userId)?.name.slice(0, 2).toUpperCase() : ""}
              </div>
              {data.users && userId ? data.users.find(user => user.id === userId)?.name : "Unknown User"}
              <span style={{ color: "gray", marginLeft: "10px" }}>
                {tickets[userId] ? tickets[userId].length : 0}
              </span>
              <img src={AddSVG} alt="Add" className="add-button" onClick={() => console.log(`Add ticket for ${userId}`)} />
              <img src={Threedot} alt="Options" className="add-button" onClick={() => console.log(`Options for ${userId}`)} />
            </p>
        ...
        {groupBy === "priority" && Object.keys(tickets).map((priority) => (
          <div key={priority} style={{ flex: "1", padding: "10px", position: "relative" }}>
            <p style={{ display: "flex", alignItems: "center" }}>
              ...
              <img src={AddSVG} alt="Add" className="add-button" onClick={() => console.log(`Add ticket with ${priorityMapping[priority]?.label} priority`)} />
              <img src={Threedot} alt="Options" className="add-button" onClick={() => console.log(`Options for ${priorityMapping[priority]?.label} priority`)} />
            </p>
